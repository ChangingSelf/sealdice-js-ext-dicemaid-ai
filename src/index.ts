import { diceMaidQQ,apiKey } from "./key";

/**
 * 本部分需要JS扩展的使用者填写自己的参数
 */
//brainshop.ai的api所需参数，注册后即可免费获得
const BID = apiKey.bid;//填入你的骰娘的大脑的id
const KEY = apiKey.key;//填入你的key

const NOT_COMMAND_FLAG = true;//若为true，则可以在@骰娘时不用指令就可以聊天
const QQ = diceMaidQQ;//骰娘自身QQ号（不带“QQ:”前缀，纯数字），用于检查自己是否被@，如果不开启无指令聊天则可以留空。由于海豹没有开放“获取骰娘自身QQ号”的API，所以只能让你们自己填写了
 ...
 * @param ext 扩展信息对象
 */
function notCommandChat(ext: seal.ExtInfo) {
